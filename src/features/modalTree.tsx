...
const TreeFilter = ({node ,callbackFilter} : NodeFilterProps )  => {
    ...
    node.children = node.children?.filter(child => TreeFilter({node: child, callbackFilter}));
    return true;
}

const ModalTree = ({tree, callbackFilter} : ModalTreeProps) => {
    const [data, setData] = useState(tree.filter(n => TreeFilter({node: n, callbackFilter})));
